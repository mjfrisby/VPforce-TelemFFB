...
class DudeHelicopter(Helicopter):
    ...
    def msfs_update_heli_controls(self, telem_data):
        ...
        if self._sim_is_msfs():
            if (
                    self.custom_ft_sw_var_enabled and self.anything_has_changed("custom_ft_sw_var",
                                                                                self.custom_ft_sw_var)
                ...
                if not self.cyclic_spring_init:
                    ...
                    if (self.cpO_x / 4096 - 0.15 < phys_x < self.cpO_x / 4096 + 0.15) and (
                        ...
                        if self._sim_is_msfs():
                            ...
                            if not telem_data.get("DudeCyclicFriction", 0):
                                ...
                                self._simconnect.send_event_to_msfs("L:H500C_OH6A-AXIS_CYCLIC_LATERAL", self.last_pos_x_pos)
                                self._simconnect.send_event_to_msfs("L:H500C_OH6A-AXIS_CYCLIC_LONGITUDINAL", self.last_pos_y_pos)
                ...
                if self.cyclic_spring_init or not (self.spring_mode_is(SpringModeEnum.FORCETRIM) and force_trim_active):
                    if self._sim_is_msfs():
                        ...
                        if not telem_data.get("DudeCyclicFriction", 0):
                            ...
                            self._simconnect.send_event_to_msfs("L:H500C_OH6A-AXIS_CYCLIC_LATERAL", pos_x_pos)
                            self._simconnect.send_event_to_msfs("L:H500C_OH6A-AXIS_CYCLIC_LONGITUDINAL", pos_y_pos)
